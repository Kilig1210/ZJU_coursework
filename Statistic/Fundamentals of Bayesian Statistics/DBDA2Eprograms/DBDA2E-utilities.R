...
openGraph = function(width=7, height=7, mag=1.0, saveName="graph", saveType="png", ...) {
  # 设置图形文件的保存路径
  if (saveType == "png") {
    filename <- paste(saveName, ".png", sep="")
    png(filename, width=width * mag, height=height * mag, ...)
  } else if (saveType == "pdf") {
    filename <- paste(saveName, ".pdf", sep="")
    pdf(filename, width=width * mag, height=height * mag, ...)
  } else if (saveType == "jpg") {
    filename <- paste(saveName, ".jpg", sep="")
    jpeg(filename, width=width * mag, height=height * mag, ...)
  } else {
    stop("Unsupported saveType. Please use 'png', 'pdf', or 'jpg'.")
  }
}
